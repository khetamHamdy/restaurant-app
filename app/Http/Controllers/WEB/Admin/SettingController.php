...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        foreach ($locales as $locale) {
            ...
            $roles['footer_text_' . $locale] = 'required';
        }

        $this->validate($request, $roles);

        foreach ($locales as $locale) {
            $setting->translate($locale)->description = $request->get('description_' . $locale);
            $setting->translate($locale)->footer_text = $request->get('footer_text_' . $locale);
        }
        ...
        if ($request->hasFile('silder_video')) {
            $name = time() . "_" . rand(10000, 99999) . "." . $request->file('silder_video')
                    ->getClientOriginalExtension();
            $request->file('silder_video')->move("uploads/settings/", $name);
            $setting->silder_image = $name;
        }
