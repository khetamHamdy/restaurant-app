                    ...
                    <form class="form" method="post" action="{{url(app()->getLocale().'/admin/settings/')}}"
                        ...
                        <div class="card-body">
                            ...
                            @foreach($locales as $locale)
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>{{__('cp.description')}}
                                            <span class="text-danger">({{$locale->name}})</span>
                                        </label>
                                        <input type="text" class="form-control form-control-solid"
                                               name="description_{{$locale->lang}}" value="{!! @$item->translate($locale->lang)->description!!}" required/>
                                    </div>
                                </div>
                            @endforeach
                            @foreach($locales as $locale)
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>{{__('cp.footer_text')}}  <span class="text-danger">({{$locale->name}})</span></label>
                                        <input type="text" class="form-control form-control-solid"
                                               name="footer_text_{{$locale->lang}}" value="{!! @$item->translate($locale->lang)->footer_text!!}" required/>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="fileinputForm">
                                        <label>{{__('cp.silder_video')}}</label>
                                        <div class="fileinput-new thumbnail"
                                             onclick="document.getElementById('edit_image4').click()"
                                             style="cursor:pointer">
                                            <video width="500px" height="500px" src="{{$item->silder_image}}" id="editImage4"></video>
                                        </div>
                                        <div class="btn btn-change-img red"
                                             onclick="document.getElementById('edit_image4').click()">
                                            <i class="fas fa-pencil-alt"></i>
                                        </div>
                                        <input type="file" class="form-control" name="silder_video"
                                               id="edit_image4"
                                               style="display:none">
                                    </div>
                                </div>
